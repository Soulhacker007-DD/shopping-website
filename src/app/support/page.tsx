  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (targetVendorId) {
          const target = fetchedUsers.find((u: any) => String(u._id) === String(targetVendorId));
            ...
            <>
              ...
              <header className="p-8 border-b border-white/5 flex items-center justify-between bg-black/20">
                ...
                <div className="flex gap-4">
                   <button className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:text-white transition-all">
                      <Search size={18} />
                   </button>
                   <button className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:text-white transition-all">
                ...
                <AnimatePresence initial={false}>
                  {messages.map((msg, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className="w-10 h-10 rounded-2xl overflow-hidden border border-white/10 shadow-lg flex-shrink-0">
                          <Image 
                        ...
                        <div className={`space-y-2 max-w-[65%] ${isMe ? "items-end" : "items-start"}`}>
                           <div className={`px-8 py-5 text-[13px] font-bold leading-relaxed shadow-xl
                             ${isMe 
                               ? "bg-blue-600 text-white rounded-[2.5rem] rounded-br-[0.5rem] border border-blue-400/20" 
                               : "bg-white/10 backdrop-blur-xl text-gray-200 rounded-[2.5rem] rounded-bl-[0.5rem] border border-white/10"
                             }`}
                           >
                             {msg.text}
                           </div>
              ...
              <footer className="p-8 border-t border-white/5 bg-black/40 space-y-6">
                ...
                <div className="relative group">
                   ...
                   <div className="relative flex items-center gap-3 bg-black/60 border border-white/10 p-3 rounded-[2.5rem] focus-within:border-blue-500/50 transition-all shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)]">
                      <button className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:text-white transition-all">
                         <Sparkles size={18} />
                      </button>
                      <input
                        value={text}
